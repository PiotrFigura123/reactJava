...
public class ExternalServiceClient extends AbstractHttpClient {
    ...
    private Flux<String> toResponse(HttpClientResponse httpClientRequest, ByteBufFlux flux) {
        return switch (httpClientRequest.status().code()) {
            case 200 -> flux.asString();
            case 400 -> Flux.error(new CleintError());
